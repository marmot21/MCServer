...
void cRedstoneSimulator::WakeUp(int a_BlockX, int a_BlockY, int a_BlockZ, cChunk * a_Chunk)
{
	...
	for (int y = -4; y <= 4; y++) for (int x = -4; x <= 4; x++) for (int z = -4; z <= 4; z++)
	{
		...
		if (
			(y < 0) ||
			(y > cChunkDef::Height) ||
			(!a_Chunk->UnboundedRelGetBlockType(RelX + x, a_BlockY + y, RelZ + z, BlockType))
			)
		{
			continue;
		}
...
void cRedstoneSimulator::Simulate(float a_Dt)
{
	...
	for (BlockList::iterator itr = m_Blocks.begin(); itr != m_Blocks.end(); ++itr)
	{
		...
		switch (BlockType)
		{
			case E_BLOCK_BLOCK_OF_REDSTONE:		HandleRedstoneBlock(*itr);	break;
			case E_BLOCK_LEVER: 				HandleRedstoneLever(*itr);	break;
			case E_BLOCK_TNT:					HandleTNT(*itr);			break;
			case E_BLOCK_REDSTONE_WIRE:			HandleRedstoneWire(*itr);	break;
...
void cRedstoneSimulator::HandleRedstoneTorch(const Vector3i & a_BlockPos, BLOCKTYPE a_MyState)
{
	...
	if (a_MyState == E_BLOCK_REDSTONE_TORCH_ON)
	{
		...
		if (m_World.GetBlockMeta(a_BlockPos) != 0x5) // Is torch standing on ground? If not (i.e. on wall), power block beneath
		{
		...
		switch (GetWireDirection(a_BlockPos))
		{
			...
			case REDSTONE_X_POS:
			{
				if (g_BlockIsSolid[m_World.GetBlock(Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z))])
				{
					...
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z - 1), Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z), a_BlockPos, E_BLOCK_REDSTONE_WIRE, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x + 1, a_BlockPos.y + 1, a_BlockPos.z), Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z), a_BlockPos, E_BLOCK_REDSTONE_WIRE, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x + 1, a_BlockPos.y - 1, a_BlockPos.z), Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z), a_BlockPos, E_BLOCK_REDSTONE_WIRE, MiddleBlock);
				}
				break;
			}
			...
			case REDSTONE_Z_POS:
			{
				if (g_BlockIsSolid[m_World.GetBlock(Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z + 1))])
				{
					...
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x - 1, a_BlockPos.y, a_BlockPos.z + 1), Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z + 1), a_BlockPos, E_BLOCK_REDSTONE_WIRE, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x, a_BlockPos.y + 1, a_BlockPos.z + 1), Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z + 1), a_BlockPos, E_BLOCK_REDSTONE_WIRE, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x, a_BlockPos.y - 1, a_BlockPos.z + 1), Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z + 1), a_BlockPos, E_BLOCK_REDSTONE_WIRE, MiddleBlock);
				}
				break;
			}
		...
		switch (a_Meta & 0x3) // We only want the direction (bottom) bits
		{
			case 0x0:
			{
				...
				if (g_BlockIsSolid[m_World.GetBlock(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z - 1)])
				{
					...
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x - 1, a_BlockPos.y, a_BlockPos.z - 1), Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z - 1), a_BlockPos, E_BLOCK_REDSTONE_REPEATER_ON, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x, a_BlockPos.y + 1, a_BlockPos.z - 1), Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z - 1), a_BlockPos, E_BLOCK_REDSTONE_REPEATER_ON, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x, a_BlockPos.y - 1, a_BlockPos.z - 1), Vector3i(a_BlockPos.x, a_BlockPos.y, a_BlockPos.z - 1), a_BlockPos, E_BLOCK_REDSTONE_REPEATER_ON, MiddleBlock);
				}
				break;
			}
			case 0x1:
			{
				if (g_BlockIsSolid[m_World.GetBlock(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z)])
				{
					...
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z - 1), Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z), a_BlockPos, E_BLOCK_REDSTONE_REPEATER_ON, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x + 1, a_BlockPos.y + 1, a_BlockPos.z), Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z), a_BlockPos, E_BLOCK_REDSTONE_REPEATER_ON, MiddleBlock);
					SetBlockLinkedPowered(Vector3i(a_BlockPos.x + 1, a_BlockPos.y - 1, a_BlockPos.z), Vector3i(a_BlockPos.x + 1, a_BlockPos.y, a_BlockPos.z), a_BlockPos, E_BLOCK_REDSTONE_REPEATER_ON, MiddleBlock);
				}
				break;
			}
...
{
	if ((m_World.GetBlockMeta(a_BlockPos) & 0x08) == 0x08)
	{
		...
		if (
			(a_BlockPos.y - 1 >= 0) &&
			((m_World.GetBlock(a_BlockPos) == E_BLOCK_WOODEN_DOOR) || (m_World.GetBlock(a_BlockPos) == E_BLOCK_IRON_DOOR)) &&
			((m_World.GetBlockMeta(a_BlockPos.x, a_BlockPos.y - 1, a_BlockPos.z) & 0x08) == 0)
			)
		{
			if ((m_World.GetBlockMeta(a_BlockPos.x, a_BlockPos.y - 1, a_BlockPos.z) & 0x04) == 0) // Closed door?
			{
				if (AreCoordsPowered(a_BlockPos)) // Powered? If so, toggle open
				{
			...
			{
				if (!AreCoordsPowered(a_BlockPos)) // Unpowered? Close if so
				{
					cBlockDoorHandler::ChangeDoor(&m_World, a_BlockPos.x, a_BlockPos.y, a_BlockPos.z);
				}
			}
	...
	{
		if ((m_World.GetBlockMeta(a_BlockPos) & 0x04) == 0) // Closed door?
		{
			if (AreCoordsPowered(a_BlockPos)) // Powered? If so, toggle open
			{
				cBlockDoorHandler::ChangeDoor(&m_World, a_BlockPos.x, a_BlockPos.y, a_BlockPos.z);
...
void cRedstoneSimulator::HandleRail(const Vector3i & a_BlockPos, BLOCKTYPE a_MyType)
{
	switch (a_MyType)
	{
		case E_BLOCK_DETECTOR_RAIL:
		{
			if ((m_World.GetBlockMeta(a_BlockPos) & 0x08) == 0x08)
			{
				...
				{
					// Power everything
					SetBlockPowered(Vector3i(a_BlockPos.x + gCrossCoords[i].x, a_BlockPos.y + gCrossCoords[i].y, a_BlockPos.z + gCrossCoords[i].z), a_BlockPos, a_MyType);
				}
			}
			break;
		}
		...
		case E_BLOCK_POWERED_RAIL:
		{
			if (AreCoordsPowered(a_BlockPos))
			{
				m_World.SetBlockMeta(a_BlockPos, m_World.GetBlockMeta(a_BlockPos) ^ 0x08);
			}
			else
			{
				m_World.SetBlockMeta(a_BlockPos, m_World.GetBlockMeta(a_BlockPos) ^ 0x08);
